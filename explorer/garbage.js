...
var promptNoteText_3 = "In the roughly twenty years that Tethys-based GAStech has been operating a natural gas production site in the island country of Kronos, it has produced remarkable profits and developed strong relationships with the government of Kronos. However, GAStech has not been as successful in demonstrating environmental stewardship. " + 
                       "<br><br> In January, 2014, the leaders of GAStech are celebrating their new-found fortune as a result of the initial public offering of their very successful company. In the midst of this celebration, <b> several employees of GAStech go  missing </b>. An organization known as the Protectors of Kronos (POK) is suspected in the disappearance, but things may not be what they seem. " + 
                       "<br><br>  As an analyst, you have a set of current and historical news reports at your disposal. You are being counted on to bring law enforcement up to date on the current organization of the POK and how that organization has changed over time, as well as to <b> characterize the events surrounding the disappearance </b>.";

var promptNoteText_derek = "The following documents all relate to arms dealing between countries across the world." +
							"<div><br></div>" +
							"Tell us your own interpretation of the dataset.";


var promptNoteText_test = " This is an example of our data analysis software which you're going to work with. Below  is a prompt note example which you will see at the beginning of your study. Prompt note gives you some clues to start your investigations" + 
						  " <br> <br> The purpose of this study is to study visualization factors relevant to understanding how people use tools to conduct data analysis." +
                          " Participants are needed to help us learn about data analysis software. You are being asked to be in this study because you expressed interest in participation. " +
                          " <br><br>  The approximate duration of this study is one to two hours. You will be asked to complete a background questionnaire about yourself. We will then explain a data analysis activity and demonstrate how to use the analysis software. Next, you will conduct a data analysis session while we use video and audio recording to collect data on how you use the software. Lastly, we will interview you to ask questions about your approach and experience."; 
...
var promptNoteText = promptNoteText_derek;
...
;(function() {
	...
	Mousetrap.bind('ctrl+l', function(){
		...
		else
		{
			...
			// Now write a log for all existing notes based on note#  
						...
						logData("Notes", "Note" + tempCounter ,"Note" + tempCounter, doc_id,text);
						tempCounter--;
						}
		}
    ...
    //logData("mouseenter-document-minimized", docDialog.attr("id"), docDialog.attr("id"), doc_id,text);
      ...
      // Send the log to its destiny
	   if (log_url != 'http://localhost:8080/log') sendLogData(jsonMessage);    // if address is the localhost, don't attampt $.ajax		
...
// give me the Note, take the title&content
	...
	function handleDocBeforeRestore (evt) {
		...
		logData("open-document", docDialog.attr("id"), docDialog.attr("id"), doc_id,text); // error 
	...
	*/
	...
	function handleDocBeforeCollapse(evt) {
		...
		var text = docDialogToText(docDialog);
		var doc_id = docDialog.find(".doc-content").attr("document_id");
		logData("collapse-document", docDialog.attr("id"), docDialog.attr("id"), doc_id,text);
	...
	function logScrunchStuff(scrunchDiv, docDialog) {
		...
		if(scrunchDiv != 0 && scrunchDiv.length != 0) {
			...
			if (scrunchStuff.length != 0) {
				...
				var text = docDialogToText(docDialog);
				var doc_id = docDialog.find(".doc-content").attr("document_id");
				logData("scrunch-highlight-view", docDialog.attr("id"), docDialog.attr("id"), doc_id,$(scrunchStuff[i]).text());
			}
		}
	}
		...
		var currentDocDiv = docDialog.find(".doc-set");			// get the doc content div
	  	...
	  	if (isOpen)
	  	{
		  	if (currentDocDiv.attr( "data-id") == scrunchOriginal) {
				...
				var text = docDialogToText(docDialog);
				var doc_id = docDialog.find(".doc-content").attr("document_id");
				logData("restore-from-scrunch", docDialog.attr("id"), docDialog.attr("id"), doc_id,text);
	...
	console.log("DOCUMENT READY");
			...
			var doccc = $(this).dialog(
				...
				dialogClass: "close",  // "dlg-no-close"
			 	...
			 	dragStart: function(event, ui) { 
						...
						var text = docDialogToText(docDialog);
						var doc_id = docDialog.find(".doc-content").attr("document_id");
						logData("startdrag-document", docDialog.attr("id"), docDialog.attr("id") , doc_id,text);
			 		},
			 		...
			 		dragStop: function() { 
			 			...
			 			var text = docDialogToText(docDialog);
						var doc_id = docDialog.find(".doc-content").attr("document_id");
						logData("enddrag-document", docDialog.attr("id"), docDialog.attr("id"), doc_id, event.clientX.toString()+ " " + event.clientY.toString());
		...
		$("div.ui-dialog").bind("mouseenter",function(){ // event,ui){  // $("div[role='dialog']").bind("mouseenter",function(event,ui){
	  				...
	  				if (docDialog.hasClass("ui-resizable")){
			 			...
			 			var text = docDialogToText(docDialog);
						var doc_id = docDialog.find(".doc-content").attr("document_id");
						logData("mouseenter-document", docDialog.attr("id"), docDialog.attr("id"), doc_id,text);
		...
		$("div[role='dialog']").bind("mouseleave",function(){  // $("div[role='dialog']").bind("mouseexit",function(event,ui){
		     	 ...
		     	 // console.log("mouose enter...");
	  				...
	  				if (docDialog.hasClass("ui-resizable")){			 			 
			 			var text = docDialogToText(docDialog);
						var doc_id = docDialog.find(".doc-content").attr("document_id");
						logData("mouseexit-document", docDialog.attr("id"), docDialog.attr("id"), doc_id,text);
					//		console.log("mouseenter-leave");
			 		}else{
			 			
			 			var text = docDialogToText(docDialog);
						var doc_id = docDialog.find(".doc-content").attr("document_id");
						logData("mouseexit-document-minimized", docDialog.attr("id"), docDialog.attr("id"), doc_id,text);
        ...
        jsPlumbInstance.bind("connection", function(info) {
			...
			logData("newConnection", doubleid, doubleid, doubleDocId  , text);
		...
		var createNote = function (noteHtml) {
            ...
            //logData("createNote", noteId + "," + mouseX + "," + mouseY, noteId);
						...
						logData("createNote", noteId + "," + mouseX + "," + mouseY,"Note" + (noteIdCounter -1), "MyNotes " + (noteIdCounter - 1));
